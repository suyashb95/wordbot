...
def format_definitions(word_data):
    ...
    for definition in word_data['definitions']:
        def_string = '{} : {}'.format(italics(definition.partOfSpeech), definition.text.encode('utf-8'))
        message += '{}\n\n'.format(def_string)
    return message
...
def format_synonyms(word_data):
    if not word_data['synonyms']: return 'No synonyms found'
...
def format_antonyms(word_data):
    if not word_data['antonyms']: return 'No antonyms found'
...
def format_urbandictionary(word_data):
    message = '{}\n{}\n\n{}\n{}'.format(
        bold('Definition'), word_data['definition'].encode('utf-8'), bold('Example'), word_data['example'].encode('utf-8')
    )
    return message
